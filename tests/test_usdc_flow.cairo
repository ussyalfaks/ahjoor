...
// Simple test to verify USDC contribution flow compiles and basic logic works
#[test]
fn test_usdc_contribution_basic_flow() {
    // Deploy contracts
...
// Test that contribution fails without proper USDC setup
#[test]
#[should_panic(expected: ('ERC20: insufficient allowance',))]
fn test_contribution_fails_without_usdc() {
    // Deploy contracts
    let owner: ContractAddress = 0x999.try_into().unwrap();
    let organizer: ContractAddress = 0x123.try_into().unwrap();
    let participant1: ContractAddress = 0x456.try_into().unwrap();
    ...
    // Try to contribute without USDC balance/approval - should fail
